...
response_window_trad_test <- subset_by_window(data_trad_test, 
                                    window_start_time = 3200, #200ms after noun onset
...
#-----------------Subset Trad dataset to Test window for only the first utterance (for reviewer response)

first_utterance_window_trad_test <- subset_by_window(data_trad_test, 
                                    window_start_time = 3200, #200ms after noun onset
                                    window_end_time = 4200, #200 ms after first utterance offset
                                    rezero = FALSE)
...
#-----------------Subset Updated dataset to Test window for only first utterance (for reviewer response)

first_utterance_window_updated_test <- subset_by_window(data_updated_test, 
                                    window_start_time = 3200, #200ms after noun onset
                                    window_end_time = 4200, #200 ms after first utterance offset
                                    rezero = FALSE)
...
#------------------t-test data, first utterance only

first_utterance_data_ttest_trad_test <- describe_data((first_utterance_window_trad_test %>% 
                                                         filter(id %in% data_ttest_trad_test$id)), 
                describe_column = "target_AOI", 
                group_columns = c("id", "lang_group", "familiarity", "trial_type", "target_word", "best_lang", "per_carrier_lang", "condition", "stimulus_to_baby", "trad_analysis_group", "first_word_presented_training", "experiment")) %>%
  mutate(total_ms = (1000/60)*N) %>%
  mutate(looking_ms = total_ms * Mean) %>%
  arrange(lang_group) %>%
  filter(!is.nan(looking_ms))
...
data_prop_by_word_trad_test <- make_time_window_data(data_trad_test, 
                                                    aois = "target_AOI",
                                                    predictor_columns = c("lang_group",  "trial_type", "per_carrier_lang", "condition", "trad_analysis_group",  "stimulus_to_baby", "first_word_presented_training", "experiment"),
                                                    summarize_by = c("id", "target_word", "familiarity"), 
                                                    other_dv_columns = c("pupil_left", "pupil_right")) 
...
#-------------------------------------------------Test WITH UTTERANCE WINDOWS

# aggregate for each participant on each trial
utterances_data_lmer_updated_test_by_trial <- 
  data_updated_test %>% 
  mutate(utterance_repetition = case_when(trial_from_zero >= 3200 & trial_from_zero <= 4200 ~ 1,
                                          trial_from_zero >= 5400 & trial_from_zero <= 6400 ~ 2,
                                          trial_from_zero >= 7600 & trial_from_zero <= 8600 ~ 3)) %>%
  filter(!is.na(utterance_repetition)) %>%
  make_time_window_data(data = .,
                        aois = "target_AOI",
                        predictor_columns = c("lang_group", "familiarity", "trial_type", "best_lang", "per_carrier_lang",
                                              "condition", "target_word", "stimulus_to_baby",
                                              "per_fr", "per_eng", 
                                              "cdi_comp_eng", "cdi_comp_fr", "total_vocab_comp",
                                              "total_looking_time_training", "utterance_repetition"),   
                        other_dv_columns = c("pupil_left", "pupil_right")) %>% 
...
#-------------------------------------------------FIRST UTTERANCE Dual lang condition against 50% chance
first_utterance_data_ttest_trad_test %>%
  filter(condition == "dual_lang") %>% 
  group_by(trad_analysis_group, familiarity, condition, experiment) %>%
  do(broom::tidy(t.test(.$Mean, mu = .5, data = .))) %>%
  mutate(cohen_d = statistic/sqrt(parameter +1)) 

#-------------------------------------------------FIRST UTTERANCE Single lang condition against 50% chance 
first_utterance_data_ttest_trad_test %>%
  filter(condition != "dual_lang") %>% 
  group_by(trad_analysis_group, condition, familiarity, experiment) %>%
  do(broom::tidy(t.test(.$Mean , mu = .5, data = .))) %>%
...
ttest_dual <- data_ttest_trad_test %>%
...
ttest_single <- data_ttest_trad_test %>%
  left_join(baseline_preference_trad, by = "id") %>% 
  mutate(compare_value = case_when(target_word == "kem" ~ Mean - preference_kem,
                                   target_word == "bos" ~ Mean - preference_bos)) %>% 
  filter(condition != "dual_lang") %>% 
...
#-------------------------------------------------FIRST UTTERANCE Dual lang condition, prop looking minus prename preference compared to zero
first_utterance_data_ttest_trad_test %>%
  left_join(baseline_preference_trad, by = "id") %>% 
  mutate(compare_value = case_when(target_word == "kem" ~ Mean - preference_kem,
                                   target_word == "bos" ~ Mean - preference_bos)) %>% 
...
#-------------------------------------------------FIRST UTTERANCE Single lang condition, prop looking minus prename preference against zero
first_utterance_data_ttest_trad_test %>%
  left_join(baseline_preference_trad, by = "id") %>% 
  mutate(compare_value = case_when(target_word == "kem" ~ Mean - preference_kem,
                                   target_word == "bos" ~ Mean - preference_bos)) %>% 
...
data_ttest_trad_test %>%
  ...
  filter(condition == "dual_lang") %>% 
  group_by(trad_analysis_group, familiarity, condition, experiment) %>%
  summarize(mean_compare_value = round(mean(compare_value), 2),
            sd_compare_value = round(sd(compare_value), 2),
            n = length(unique(id)))
...
lm_all <- lm(Prop - .5 ~ target_word + per_exposure_target + vocab_target + total_looking_time_training, 
             data = data_lmer_updated_test_by_trial)
summary(lm_all)
...
#---------------------------All predictors SPLIT UTTERANCE WINDOWS

utterances_lmer_rand_it_all_cor <- lme4::lmer(difference_score ~ per_exposure_target + vocab_target + total_looking_time_training + utterance_repetition + (1|target_word), 
                        data = utterances_data_lmer_updated_test_by_trial) 

#---------------------------All predictors FIRST UTTERANCE ONLY

first_utterance_lmer_rand_it_all_cor <- utterances_data_lmer_updated_test_by_trial %>%
  filter(utterance_repetition == 1) %>%
  lme4::lmer(difference_score ~ per_exposure_target + vocab_target + total_looking_time_training + (1|target_word), 
                        data = .) 
...
#---------------------------Simple Linear Model SPLIT BY UTTERANCE WINDOWS

utterances_lm_target_word <-
  utterances_data_lmer_updated_test_by_trial %>%
  lm(difference_score ~ target_word + per_exposure_target + vocab_target + total_looking_time_training + utterance_repetition, data = .)

summary(utterances_lm_target_word)
...
tab_model(utterances_lm_target_word)
...
tab_model(utterances_lmer_rand_it_all_cor)

tab_model(first_utterance_lmer_rand_it_all_cor)
...
#---------------------Get significance info from t-test section... 

sig_text <- ttest_dual %>%
  filter(p.value <= .05) %>%
  mutate(x_labels = str_to_sentence(str_replace(experiment, "_", " ")),
         familiarity = str_to_sentence(str_replace(familiarity, "_", " ")),
         y = estimate + .1,
         lab = case_when(p.value < .01 ~ paste0("**", str_remove(round(p.value, 3), "^0")),
                         TRUE ~ paste0("*", str_remove(round(p.value, 3), "^0")))) %>%
  select(x_labels, familiarity, y, lab)
...
#Review responses
```{r}
#plot looking by utterance repetition
#OVERALL

utterances_data_lmer_updated_test_by_trial %>%  
  filter(!is.nan(difference_score)) %>%  
  ggplot(aes(x  =  utterance_repetition, y  = difference_score)) + 
  geom_point(alpha = .1) + 
  stat_summary(geom  = "point") +  
  stat_summary(geom  = "line") +
  scale_x_continuous(breaks = c(1,2,3))

#SPLIT BY FAMILIARITY AND TARGET WORD

utterances_data_lmer_updated_test_by_trial %>%  
  filter(!is.nan(difference_score)) %>%  
  ggplot(aes(x  =  utterance_repetition, y  = difference_score, color =  target_word)) + 
  geom_point(alpha = .1) + 
  stat_summary(geom  = "point") +  
  stat_summary(geom  = "line") +
  facet_grid(familiarity~target_word) + 
  scale_x_continuous(breaks = c(1,2,3))

#PLOT TIME COURSE OF LOOKING
#uncorrected
data_trad_test %>%
  ungroup() %>%
  mutate(time_bin = trial_from_zero %/% 250-11,
         utterance = case_when(time_bin >= 1 & time_bin <= 4 ~ 1,
                                          time_bin >= 10 & time_bin <= 13 ~ 1,
                                          time_bin >= 19 & time_bin <= 22 ~ 1,
                               TRUE ~ 0)) %>% 
  group_by(familiarity, experiment, target_word, condition, time_bin, utterance) %>%
  summarize(looking = mean(target_AOI, na.rm = T),
            n = n_distinct(id)) %>%
  ungroup() %>%
  mutate(n = case_when(time_bin == max(time_bin) ~ paste0("n: ", n), TRUE ~ "")) %>%
  ggplot(aes(x = time_bin, y = looking, color = familiarity)) +
  geom_point() +
  geom_line() +
  ...
  annotate("rect", fill = "red", alpha = 0.25, 
        xmin = 19, xmax = 22,
        ymin = -Inf, ymax = Inf) +
  facet_grid(experiment ~ target_word)
