...
trackloss_updated_training <- trackloss_analysis(response_window_updated_training)

data_updated_training <- clean_by_trackloss(data = response_window_updated_training,
                                   trial_prop_thresh = (1 - 750/(11000 - 3700))) # Need at least 750ms looking = 750/(11000 - 3700) divide by trial length

not_enough_training_data_ids <- response_window_updated_training %>% distinct(id) %>% anti_join(data_updated_training %>% distinct(id))
...
#-----------------------First, update final dataset
data_trad_test <- data_trad_test %>%
  filter(id %notin% not_enough_training_data_ids$id)
...
response_window_trad_training %>% 
  ...
  summarize(trial_length = length(id), #Check this
