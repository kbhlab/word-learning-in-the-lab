...
#1.1. Load dataset
```{r}
load(here("data/2022-02-10_data_ready_for_analysis.Rda"))
#write.csv(dataset, file = "data_ready_for_analysis.csv") # generates a 1.6GB file

dataset <- dataset %>%
   mutate(exclude = case_when(id == "Novel_14_S124" & trial_number == 6 ~ TRUE, #remove these trials for technical lagging (When we plotted the data, these trials took longer than they should have and so TOBII lagged. Remove them)
...
#participants <- read_csv(here("data", "OLD_participants_data.csv")) %>% 
  filter(!is.na(baby_id))#288 total 

da_estrela_keepers <- read_csv(here("data", "da_estrela_original_keepers.csv"))

```
#1.3. Counting number of excluded participants and the reason for exclusion
```{r}
participants %>%
...
```
#1.4 Create language groupings

```{r}
#------------------------------------------------------Create a zeroed timestamp and language groupings
...
```
#2.2. Create variable "stimulus_to_baby" depending on whether the stimulus is in the infant's dominant language, non-dominant or in a foreign language
```{r}
data_keepers_trad <- data_keepers_trad %>% 
...
prename_pref_window_test_trad <- subset_by_window(data_keepers_trad_test, 
...
participants %>% filter(is.na(reason_exclusion)) %>% nrow() - n_distinct(response_window_test_trad$id) #number of participants excluded due to trackloss
...
final_sample_ids <- data_test_trad %>% select(id, experiment) %>% distinct() %>% mutate(id = as.character(id)) %>% ungroup() #110 participants
...
#How many had en vs fr as nondom
final_sample %>%
  filter(lang_group == "bilingual_other_nondom") %>%
  count(best_lang)
...
# For kids with any exposure to a thrid language, what are the stats?

participants %>% 
  filter(recording_name %in% final_sample_ids$id) %>% 
  filter(lang_group != "Monolingual") %>%
  filter(per_eng>0 & per_fr>0 & per_other>0) %>%
  pivot_longer(per_eng:per_other, names_to = "lang_source", values_to = "per_exposure") %>%
  group_by(recording_name) %>%
  mutate(min_lang = min(per_exposure)) %>% 
  ungroup() %>%
  pivot_wider(names_from = "lang_source", values_from = "per_exposure") %>%
  summarize(mean=mean(min_lang),
            sd=sd(min_lang))


```
## Prenaming Preferences

```{r}

prename_data <- prename_pref_window_test_trad %>%
  filter(id %in% final_sample_ids$id) %>%
...
baseline_preference <- prename_data %>%
  select(id, object, prop) %>%
  pivot_wider(names_from = "object", values_from = "prop") %>%
  rename(preference_kem = kem, 
         preference_bos = bos)

#do it with proportion instead of average ms looking time
#can account for prenaming pref by subtracting each child's preference score from themselves before running models - diff between Prop and baseline score for each child (their baseline, ex. would be bos 45% of time in prenaming phase. How much more did they look at KEM than their baseline (prenaming) - per kid) For each kid, baseline preference score, for each trial, subtract performance from baseline. Comapre to 0 instead of 50%.
...
#instead of Mean, use corrected percentage compared to baseline, instead of .5 compare to 0
data_test_participant_summary_trad %>%
  filter(condition == "dual_lang") %>% 
...
#3.3.35. Run t-tests corrected for prenaming preference against 0 for each language group in each condition
```{r}

#instead of Mean, use corrected percentage compared to baseline, instead of .5 compare to 0
data_test_participant_summary_trad %>%
  left_join(baseline_preference, by = "id") %>% 
...
# from Audacity, looks like "bos" utterances occur at 3000ms, 5450ms, and 7850ms.
# for "kem" utterances, it's 3150ms, 5500ms, and 7810ms

utterances <- tibble(target_word = c(rep("kem", 3), rep("bos", 3)), word_utterance = c(rep(1:3, 2)), onset = c(rep(c(3075, 5475, 7830), 2)), offset = c(rep(c(3550, 6075, 8425), 2)))

data_time_series <- data_keepers_trad_test %>%
  filter(id %in% final_sample_ids$id) %>%
  filter(trial_from_zero >= 0 & trial_from_zero <= 10000) %>%
  make_time_sequence_data(time_bin_size = 200,
                          predictor_columns = c("lang_group", "familiarity", "trial_type", "trial_number_of_type", "per_carrier_lang", "condition", "trad_analysis_group", "target_word", "first_word_presented_training", "experiment"),
                          aois = c("target_AOI", "distractor_AOI"))


data_time_series %>%
  #filter(target_word == "bos") %>%
  pivot_wider(names_from = AOI, values_from = Prop) %>%
  group_by(condition, target_word, Time) %>% 
  summarise(target = mean(target_AOI, na.rm=TRUE),
            distractor = mean(distractor_AOI, na.rm = TRUE)) %>% 
  ungroup() %>%
  mutate(text = case_when(target_word == "bos" & condition == "dual_lang" & row_number() == 5 ~ "Shaded areas indicate\ntarget word utterance",
                          TRUE ~ ""),
         condition = case_when(condition == "dual_lang" ~ "Dual Language",
                               condition == "single_lang_foreign" ~ "Single Language (Foreign)",
                               condition == "single_lang_native" ~ "Single Language (Native)"
                               )) %>%
  rename(`Target word` = target_word) %>%
  #pivot_longer(cols = c(target, distractor), names_to = "aoi", values_to = "prop_looking") %>%
  
  ggplot(aes(x = Time, y = target, color = `Target word`)) + 
  geom_hline(yintercept = .5, color = "blue") +
  geom_text(aes(label = text), x = 5000, y = .9, color = "black", show.legend = FALSE) +
  annotate("rect", xmin = utterances$onset[1], xmax = utterances$offset[1], ymin = 0, ymax = 1, fill = "#2DA2BB", alpha = .2, colour = "transparent", show.legend = FALSE) +
  annotate("rect", xmin = utterances$onset[2], xmax = utterances$offset[2], ymin = 0, ymax = 1, fill = "#2DA2BB", alpha = .2, colour = "transparent", show.legend = FALSE) +
  annotate("rect", xmin = utterances$onset[3], xmax = utterances$offset[3], ymin = 0, ymax = 1, fill = "#2DA2BB", alpha = .2, colour = "transparent", show.legend = FALSE) +
  geom_line(show.legend = FALSE) +
  facet_grid(condition~`Target word`) +
  ggtitle("Time Course of Infant Looking During Test Trials") +
  theme_pubclean()
  
#ggsave(here("figures", "time_course_plot.png"), width = 10)
...
n_distinct(data_lmer_test_eye_window_clean$id) # 148
```
## DESCRIPTIVES UPDATED ANALYSIS

```{r descriptives_updated}

#---------------------------------------------------------UPDATED

final_sample_ids_updated <- data_lmer_test_eye_window_clean %>% 
  select(id) %>% distinct() #151 participants

final_sample_updated <- final_sample_ids_updated %>%
  left_join(participants, by = c("id" = "recording_name"))

only_1_trial_in_trad <- response_window_test_trad %>% 
  anti_join(data_test_trad, by = "id") %>% 
  distinct(id) %>% 
...
#how many with each of eng or fr as best lang?
infants_added %>% distinct(id, dom_lang, best_lang) %>% count(dom_lang, best_lang)
...
data_lmer_training_eye <- data_lmer_training_eye %>%
...
#n_distinct(data_lmer_training_eye_window_clean$id) # 149

# summary of total looking time for each participant on each trial
...
# final sample
data_lmer_test_agg_id %>% distinct(id) %>% count() # 151 participants
...
plot_perc <- 
  data_lmer_test_agg_id %>% 
  mutate(condition = case_when(str_detect(condition, "one_lang") ~ "one_language",
                          str_detect(condition, "two_lang") ~ "two_language")) %>%
...
Centered DV: `looking proportion to target - chance (0.5)`, 0 corresponds to chance performance, negative indicates more looks **away** from target, positive indicate looks **towards** the target.
...
lmer_mod3.1 <- lme4::lmer(Prop - .5 ~ condition + (target_word|id), 
                        data = data_lmer_test_agg_id) # singularity warning
...
lmer_mod6.1 <- lme4::lmer(Prop - .5 ~ condition + (1|target_word) + (1|id), 
                        data = data_lmer_test_agg_id) # singularity warning
...
lmer_mod9.1 <- lme4::lmer(Prop - .5 ~ total_looking_time_training + (1|id), 
                        data = data_lmer_test_agg_id) # singularity warning
...
lmer_mod10 <- lme4::lmer(Prop - .5 ~ per_exposure_target + (1|target_word), 
                         data = data_lmer_test_agg_id) # converge 

lmer_mod11 <- lme4::lmer(Prop - .5 ~ vocab_target  + (1|target_word), 
                        data = data_lmer_test_agg_id) # converge

lmer_mod12 <- lme4::lmer(Prop - .5 ~ total_looking_time_training  + (1|target_word), 
                        data = data_lmer_test_agg_id) # converge

# single model w/ all regressors
lmer_mod13 <- lme4::lmer(Prop - .5 ~ per_exposure_target + vocab_target + total_looking_time_training + (1|target_word), 
                        data = data_lmer_test_agg_id) # converge

# diff as dv
lmer_mod10_diff <- lme4::lmer(difference_score ~ per_exposure_target  + (1|target_word), 
                         data = data_lmer_test_agg_id) # converge 

lmer_mod11_diff <- lme4::lmer(difference_score ~ vocab_target  + (1|target_word), 
                        data = data_lmer_test_agg_id) # converge

lmer_mod12_diff <- lme4::lmer(difference_score ~ total_looking_time_training  + (1|target_word), 
                        data = data_lmer_test_agg_id) # converge
...
summary(lmer_mod10_diff)
ranef(lmer_mod10_diff)

summary(lmer_mod11_diff)
ranef(lmer_mod11_diff)

summary(lmer_mod12_diff)
ranef(lmer_mod12_diff)
...
# joint table
tab_model(lmer_mod10, lmer_mod11, lmer_mod12, lmer_mod13,
          show.re.var = T,
          show.icc = T,
...
# diff models
tab_model(lmer_mod10_diff,
          show.re.var = T,
          show.icc = T,
          title = "Exposure to target (%)"#,
          #file = here("figures/table_lmer_exposure_diff.doc")
          )

tab_model(lmer_mod11_diff,
          show.re.var = T,
          show.icc = T,
          title = "Vocabulary in target language (n)"#,
          #file = here("figures/table_lmer_vocab_diff.doc")
          )

tab_model(lmer_mod12_diff,
          show.re.var = T,
          show.icc = T,
          title = "Total looking time during Training"#,
          #file = here("figures/table_lmer_training_diff.doc")
          )

# joint table
tab_model(lmer_mod10_diff, lmer_mod11_diff, lmer_mod12_diff,
          show.re.var = T,
          show.icc = T,
          title = "Joint table: % exposure, vocabulary, looking time"#,
          #file = here("figures/table_lmer_exp_vocab_training_diff.doc")
          )
...
lmer_mod10_one_lang_all <- 
  data_lmer_test_agg_id %>% 
  filter(str_detect(condition, "one_lang")) %>% 
  lme4::lmer(Prop - .5 ~ per_exposure_target + (1|target_word), 
                         data = .) # converge 


tab_model(lmer_mod10_two_lang, lmer_mod10_one_lang_all,
          show.re.var = T,
          show.icc = T,
...
# separate models for each condition - difference score/prenaming pref
# exposure
lmer_mod10_two_lang_diff <- 
  data_lmer_test_agg_id %>% 
  filter(condition == "two_lang") %>% 
  lme4::lmer(difference_score ~ per_exposure_target + (1|target_word), 
                         data = .) # converge 
lmer_mod10_one_lang_all_diff <- 
  data_lmer_test_agg_id %>% 
  filter(str_detect(condition, "one_lang")) %>% 
  lme4::lmer(difference_score ~ per_exposure_target + (1|target_word), 
                         data = .) # converge 
tab_model(lmer_mod10_two_lang_diff, lmer_mod10_one_lang_all_diff,
          show.re.var = T,
          show.icc = T,
          title = "Exposure to target (%): 2-lang, 1-lang"#,
          #file = here("figures/table_lmer_exposure_by_condition_diff.doc")
          )
...
lmer_mod11_one_lang_all <- 
  data_lmer_test_agg_id %>% 
  filter(str_detect(condition, "one_lang")) %>% 
  lme4::lmer(Prop - .5 ~ vocab_target + (1|target_word), 
                         data = .) # converge 

tab_model(lmer_mod11_two_lang, lmer_mod11_one_lang_all,
          show.re.var = T,
          show.icc = T,
          title = "Vocabulary: 2-lang, 1-lang"#,
          #file = here("figures/table_lmer_condition_vocab.doc")
          )
...
lmer_mod12_one_lang_all <- 
  data_lmer_test_agg_id %>% 
  filter(str_detect(condition, "one_lang")) %>% 
  lme4::lmer(Prop - .5 ~ total_looking_time_training + (1|target_word), 
                         data = .) # converge 

tab_model(lmer_mod12_two_lang, lmer_mod12_one_lang_all,
          show.re.var = T,
          show.icc = T,
          title = "Looking during Training: 2-lang, 1-lang"#,
          #file = here("figures/table_lmer_by_condition_training.doc")
          )
...
tab_model(lmer_null,
          ...
          #file = here("figures/table_lmer_null.doc")
          )
```
...
## 8.1. Looking while listening - Effect size
...
### 8.1.4. Observed effect size based on simulation
...
# predictors: 
pwr_sim_lmer_mod10 <- simr::powerSim(lmer_mod10, # per_exposure_target
                                     nsim = nsim) 

pwr_sim_lmer_mod11 <- simr::powerSim(lmer_mod11, # vocab_size
                                     nsim = nsim) 

pwr_sim_lmer_mod12 <- simr::powerSim(lmer_mod12, # looking during training
                                     nsim = nsim) 

# effect sizes
pwr_sim_lmer_mod10 # 0.000004
pwr_sim_lmer_mod11 # 0.00011
pwr_sim_lmer_mod12 # -0.0000001

}
```

## 8.2. Switch task - Effect size
...
### 8.3.1. Traditional analysis: t-test
...
### 8.3.1. Updated analysis: mixed-models
...
pc_lmer_mod10 <- powerCurve(lmer_mod10)
pc_lmer_mod11 <- powerCurve(lmer_mod11)
pc_lmer_mod12 <- powerCurve(lmer_mod12)
...
Here we intend to test if the effect size for word learning in our traditional (t-test) and updated (lmer) analysis is below our smallest ES of interest. We will perform one overall equivalence test for the traditional analysis and another using the multiple regression from the updated analysis.
...
For the traditional analysis, we are summarizing the data so each participant only have 1 trial. Proportion of looking to target is averaged. For the updated analysis, we are feeding the equivalence test with the fitted model directly. 

```{r}
# dataset from traditional analysis (1 trial per participant)
data_eq_trad <- 
  data_test_participant_summary_trad %>% 
  group_by(id) %>%
  summarise(prop = mean(Mean)) %>%
  ungroup()

nrow(data_eq_trad) # 110 participants
head(data_eq_trad) 
```

## 9.2. Equivalence testing

### 9.2.1. Traditional analysis

This is an approximate test. Keep in mind that the effect size estimated here will be most likely overstimated, because we use aggregated data (1 trial per participant). Better calculations of test statistics, p-values, and effect sizes are found under 7.2., when fitting pruned mixed-models. 

Following Lakens et al. (2018), we set our smallest effect size of interest (SESOI) based on the estimated ES (*d* = 0.33) of the mini meta-analysis we performed on step 8.2. We use a symmetrical interval, so our upper boundary will be *d* = 0.33 and our lower boundary will be *d* = -0.33. 

```{r}
# sesoi
ub_sesoi <- switch_mini_ma$b 
lb_sesoi <- switch_mini_ma$b * -1 

# tost: one-sample t-test against chance
equiv_test_trad <- 
  TOSTER::t_TOST(x = data_eq_trad$prop,
                 hypothesis = "EQU", # equivalence test
                 eqb = c(lb_sesoi, ub_sesoi), # boundaries
                 mu = 0.5, # chance level
                 eqbound_type = "SMD", # we use Cohen'd from MA
                 smd_ci = "goulet")

print(equiv_test_trad)
plot(equiv_test_trad)

# sanity check on ES of equivalence tests
effectsize::hedges_g(data_eq_trad$prop, mu = 0.5) # similar
```

### 9.2.2. Updated analysis

```{r}
# tost: mixed-effects multiple regression
equiv_test_upd <- 
  parameters::equivalence_test(x = lmer_mod13, # mixed-effects multiple regression
                               range = c(ub_sesoi, lb_sesoi),
                               effects = c("fixed", "random"))

print(equiv_test_upd)
plot(equiv_test_upd)
```
