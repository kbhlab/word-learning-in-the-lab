...
#1.1. Load dataset
```{r}
load(here("data/2021-11-04_data_ready_for_analysis.Rda"))
...
final_sample_ids <- data_test_trad %>% select(id, experiment) %>% distinct() %>% mutate(id = as.character(id)) %>% ungroup() #109 participants
...
n_distinct(data_lmer_test_eye_window_clean$id) # 151 
```
