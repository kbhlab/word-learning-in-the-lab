...
Here we intend to test if the effect size for word learning in our largest sample (*N* = 148; updated analysis) is below our smallest ES of interest. This is an approxiamte test. Keep in mind that the effect size estimated here will be most likely overstimated, because we use aggregated data (1 trial per participant). Better calculations of test statistics, p-values, and effect sizes are found under 7.2., when fitting pruned mixed-models. 


We use the most complete dataset (updated analysis) and summarise proportion of looking to target on test trials for each participant. Following Lakens et al. (2018), we set our smallest effect size of interest (SESOI) based on the estimated ES (*d* = 0.33) of the mini meta-analysis we performed on step 8.2. We use a symmetrical interval, so our upper boundary will be *d* = 0.33 and our lower boundary will be *d* = -0.33. 
...
```{r}
# aggregated data, 1 trial per participant
data_lmer_test_eq <- 
  data_lmer_test_agg_id %>% 
  group_by(id) %>%
...
equiv_test <- 
  TOSTER::t_TOST(x = data_lmer_test_eq$prop,
...
effectsize::hedges_g(data_lmer_eq$prop, mu = 0.5) # similar
```
